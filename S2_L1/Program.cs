...
namespace S2_L1
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            PrintToConsole(pupilsContainer, network, alreadyKnowTest, networkList, removedPupil, mutualFriends, friendsNetworkData, networkData);
            Console.WriteLine($"Pradiniai duomenys ir rezultatai spausdinami lentelėmis tekstiniame faile: {resultsFile}");
            PrintToReportTable(pupilsContainer, network, alreadyKnowTest, networkList, removedPupil, mutualFriends, friendsNetworkData, networkData, resultsFile);
            Console.ReadKey();
        }
        ...
        static string NetworkResult(PupilsContainer pupilsContainer, string firstPupil, string secondPupil, bool alreadyKnowTest, List<string> networkList, Pupil removedPupil, List<string> mutualFriends)
        {
            ...
            for (int i = 0; i < pupilsContainer.Count; i++)
            {
                ...
                if (pupilsContainer.GetPupil(i).Name == firstPupil)
                {
                    ...
                    {
                        if (!(NetworkList(networkList, removedPupil) || pupilsContainer.GetPupil(i).Friends[j] == removedPupil.Name)) // Tikrina ar ryšių sąraše nėra to pačio mokinio
                        {
                        ...
                        if (j + 1 == pupilsContainer.GetPupil(i).Friends.Count) // Tikrina ar baigėsi moksleivio draugai
                        {
                            ...
                            {
                                removedPupil = pupilsContainer.GetPupil(i);
                                pupilsContainer.RemovePupil(i);
                                return NetworkResult(pupilsContainer, networkList[0], secondPupil, alreadyKnowTest, networkList, removedPupil, mutualFriends);
                            }
                        }
                    }
                }
        ...
        /// <param name="removedPupil"> Išimtas moksleivis</param>
        /// <returns> Taip arba ne </returns>
        static bool NetworkList(List<string> networkList, Pupil removedPupil)
        {
            for (int i = 0; i < networkList.Count; i++)
            {
                if (networkList[i] == removedPupil.Name)
                {
                    return true;
                }
            }
            ...
            string friendsNetworkData, string networkData)
        {
            ...
            foreach (var pupil in network)
            {
                ...
                var result = NetworkResult(pupilsContainerCopy, pupil.FirstFriend, pupil.SecondFriend, alreadyKnowTest, networkList, removedPupil, mutualFriends);
        ...
        string friendsNetworkData, string networkData, string resultsFile)
        {
            using (var writer = new StreamWriter(resultsFile, false, Encoding.UTF8))
            {
                ...
                foreach (var friend in network)
                {
                    ...
                    var result = NetworkResult(pupilsContainerCopy, friend.FirstFriend, friend.SecondFriend, alreadyKnowTest, networkList, removedPupil, mutualFriends);
